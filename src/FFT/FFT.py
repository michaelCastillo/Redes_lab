...
def plotSpec(signal, fs_rate, title):
    ...
    #Espectograma de la señal
    plt.specgram(signal, Fs=fs_rate)

#MODULACION DE UNA FUNCION COSENO DE EJEMPLO, EN DONDE:
# LA PORTADORA ES UN COSENO CON FRECUENCIA = f Hz
# LA MODULADORA ES UN CON COSENO CON FRECUENCIA = f/2 Hz
# CON UNA FRECUENCIA DETERMINADA
def graphics(fs_rate, signal):
    freq = 5000
    #fs_rate = 4 * freq
    #La frecuencia de muestreo debe ser al menos 4 la frecuencia (Por qué ? lo lei por ahi y funciona)
    time = float(len(signal)) / float(fs_rate)
    print("TIME => " + str(time))
    #Para que se cumpla el teorema de nyquist
    fs_rate = 4 * freq
    #Se realiza la modulacion
...
def newModulation(signal, fs_rate, time, fZero):
    ...
    # Demodulacion
    plt.subplot(313)
    # Otener la demodulacion multiplicando por otra funcion coseno
